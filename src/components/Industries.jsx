...
const industries = [
  { name: "Commercial Cleaning", icon: "▥", position: "left-1/2 top-[2%] -translate-x-1/2" },
  { name: "Roofing", icon: "⌂", position: "right-[3%] top-[17%]" },
  { name: "Solar", icon: "☼", position: "right-[-1%] top-[40%]" },
  { name: "Engineering", icon: "⚙", position: "right-[4%] top-[60%]" },
  { name: "SaaS", icon: "☁", position: "right-[13%] bottom-[13%]" },
  { name: "Finance / Merchant Processing", icon: "▤", position: "left-1/2 bottom-[1%] -translate-x-1/2" },
  { name: "Real Estate", icon: "▥", position: "left-[7%] bottom-[14%]" },
  { name: "Dental", icon: "♢", position: "left-[1%] top-[60%]" },
  { name: "Local Services", icon: "✕", position: "left-[0%] top-[39%]" },
  { name: "IT Managed Services", icon: "⋔", position: "left-[7%] top-[16%]" },
];
...
export default function Industries() {
  return (
    <section
      id="industries"
      className="relative overflow-hidden border-b border-cyan-500/10 bg-[#03060b] px-5 py-20 md:px-6 md:py-24"
    >
      <div className="absolute left-1/2 top-28 h-[420px] w-[420px] -translate-x-1/2 rounded-full bg-cyan-300/10 blur-[150px] md:h-[520px] md:w-[520px]" />
      ...
      <div className="relative mx-auto max-w-7xl">
        ...
        <Reveal y={34} delay={0.15}>
          <div className="relative mx-auto mt-14 hidden aspect-[1.32/1] w-full max-w-[920px] lg:block">
            ...
            <svg className="absolute inset-0 z-10 h-full w-full overflow-visible" viewBox="0 0 660 660" fill="none" xmlns="http://www.w3.org/2000/svg">
              <defs>
                <filter id="wireGlow">
            ...
            {industries.map((industry, index) => (
              <motion.div
                ...
                transition={{ duration: 0.55, delay: 0.45 + index * 0.05, ease: [0.22, 1, 0.36, 1] }}
        ...
        <div className="mt-10 grid grid-cols-1 gap-3 min-[390px]:grid-cols-2 lg:hidden">
  {industries.map((industry, index) => (
    <Reveal
      key={industry.name}
      delay={index * 0.04}
      y={20}
    >
      <div className="flex min-h-[62px] items-center gap-3 rounded-2xl border border-cyan-300/20 bg-[#061019]/90 px-4 py-3 text-[12px] font-medium text-white shadow-[0_0_20px_rgba(34,211,238,0.08)] backdrop-blur-xl transition duration-300">

        <span className="text-lg text-cyan-300 drop-shadow-[0_0_12px_rgba(34,211,238,0.8)]">
          {industry.icon}
        </span>
        ...
        <Reveal y={24} delay={0.2}>
          <div className="mt-12 text-center">
            <Link
              href="/industries"
              className="inline-flex w-full max-w-[280px] items-center justify-center gap-3 rounded-full border border-cyan-300/45 bg-cyan-300/10 px-6 py-3 text-[13px] font-bold text-cyan-100 shadow-[0_0_28px_rgba(34,211,238,0.18)] transition duration-300 hover:border-cyan-300 hover:bg-cyan-300 hover:text-black"
            >
              Explore Industries
